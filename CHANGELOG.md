...
- `extract/fixtures/EmvParserFixturesTest.kt`: 12 tests driving every fixture through `EmvParser.parse` and asserting the expected `EmvCard` field-by-field. Pinning per-brand integration coverage that complements the focused canonical-fixture tests in `EmvParserTest`.
