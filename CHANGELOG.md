...
- Post-review remediation: fixed `startScan()` task-cancellation race + NFC session leak via strong stream capture; replaced `KotlinByteArray` test fixture with `EmvReaderTestFixtures.sampleVisaCard()` Swift bridge in the `EmvReader` package (CLAUDE.md §8 — KMP types kept off iosApp); added 5 missing friendly-message tests + PCI-safety regression test pinning `EmvCardSummary.pan` masking to the `411111******1111` form + diagnostic hex-leak regression; replaced busy-polling `waitUntil` with deterministic `await currentScanTask?.value` (DEBUG-only handle on `ReaderViewModel`); cleaned up `CardSummary` Track 2 label, switched `.onAppear` to `.task`, added explicit `Sendable` to `ReaderUiState`, documented XCFramework branch-switch footgun in README. Test-target wiring deferred to #67.
